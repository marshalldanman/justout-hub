...
(function () {
  ...
  // ============================================================
  //  1. SERVICE WORKER — DISABLED for helpdesk subdomain
  //  Unregisters any stale SW to prevent caching issues.
  //  The helpdesk subdomain uses network-only (no SW caching).
  // ============================================================
  function registerSW() {
    ...
    navigator.serviceWorker.getRegistrations().then(function (registrations) {
      registrations.forEach(function (reg) {
        reg.unregister().then(function () {
          log('Unregistered stale service worker: ' + reg.scope);
        });
      });
    });

    // Also clear any old caches from the dashboard SW
    if ('caches' in window) {
      caches.keys().then(function (names) {
        names.forEach(function (name) {
          caches.delete(name).then(function () {
            log('Cleared stale cache: ' + name);
          });
        });
      });
    }

    log('Service worker disabled — helpdesk runs network-only');
  }
